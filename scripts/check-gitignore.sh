...
mkdir -p .qa playwright/.auth .playwright-mcp-profile e2e/playwright/.auth
...
FAIL=0
# e2e/playwright/.auth/user.json là ca hồi quy: pattern neo-gốc `playwright/.auth/`
# KHÔNG khớp nó. Chứa authToken/refreshToken sống — rò là rò thật.
for f in .qa/a.md playwright/.auth/user.json .playwright-mcp-profile/p \
         x.xlsx.bak result.json staging.postman_environment.json .env \
         e2e/playwright/.auth/user.json; do
